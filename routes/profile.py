...
def profile_page():
    ...
    if profile_data and profile_data.gallery:
        try:
            gallery = json.loads(profile_data.gallery)
        except:
            gallery = []
...
def profile_setup():
    ...
    for file in gallery_files:
        if file and file.filename != '':
            ...
            gallery_paths.append(f"uploads/{filename}")

    if gallery_paths:
        existing = json.loads(profile_data.gallery or "[]")
        existing.extend(gallery_paths)
        profile_data.gallery = json.dumps(existing)
